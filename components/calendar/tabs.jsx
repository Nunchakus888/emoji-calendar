...
const ViewSwitchSelector = ({selected = operationsMap.viewList[0].value, onChange}) => {
  return (
    <select onChange={onChange} className="bg-gray-50 border border-gray-300 text-gray-900 mb-6 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-40 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
      {
        operationsMap.viewList.map(({label, value}) => (
          <option selected={value === selected} key={value} value={value}>{label}</option>
        ))
      }
  </select>
  )
